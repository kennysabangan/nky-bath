...
function buildEmailHtml(data: {
  ...
  const pathMatch = pagePath.match(/\/bath\/([^/]+)/);
  ...
  const sourceLabel = isHomePage ? 'Homepage' : `${cityDisplay} — Bathroom Refinishing`;
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const html = buildEmailHtml({
    siteName: site_name || 'Bathroom Refinishing',
    fullName,
    email,
    phone: phone || '',
    comment: comment || '',
  ...
  try {
    await resend.emails.send({
      from: 'NKY Bath Alerts <alerts@mursenmaintenance.com>',
